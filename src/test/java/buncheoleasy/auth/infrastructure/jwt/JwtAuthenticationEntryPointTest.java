  ...
  class CommenceTest {
    ...
    void 요청_속성에_BusinessException이_있으면_해당_에러코드로_응답한다() throws Exception {
      ...
      assertThat(problemDetail.getStatus())
          .isEqualTo(ErrorCode.AUTH_EXPIRED_TOKEN.getHttpStatus().value());
      assertThat(problemDetail.getProperties().get("code"))
          .isEqualTo(ErrorCode.AUTH_EXPIRED_TOKEN.getCode());
    }
    ...
    void 요청_속성에_예외가_없으면_AUTH_INVALID_TOKEN으로_응답한다() throws Exception {
      ...
      ProblemDetail problemDetail = captor.getValue();
      assertThat(problemDetail.getStatus())
          .isEqualTo(ErrorCode.AUTH_INVALID_TOKEN.getHttpStatus().value());
      assertThat(problemDetail.getProperties().get("code"))
          .isEqualTo(ErrorCode.AUTH_INVALID_TOKEN.getCode());
    }
    ...
    void 요청_속성에_BusinessException이_아닌_예외가_있어도_AUTH_INVALID_TOKEN으로_응답한다() throws Exception {
      ...
      MockHttpServletResponse response = new MockHttpServletResponse();
      request.setAttribute(
          JwtAuthenticationFilter.EXCEPTION_ATTRIBUTE, new RuntimeException("unexpected"));
